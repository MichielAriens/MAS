...
//		// TODO uiteraard nog resultaten bijhouden en GUI weglaten voor simulaties
	...
	public static void run(int modus, int numFireFighters, long seed) {
	    ...
	    if (modus == 0) {
	    	...
	    	for(int i = 0; i < numFireFighters; i++){
	    		//sim.register(new AntFireFighter(roadModel.getRandomPosition(rng), new SimpleLimitedLOS(5,5,roadModel), agvRng));
	    		sim.register(new AntFireFighter(roadModel.getRandomPosition(rng), new FullLineOfSight(), agvRng));
	    	} 
	    }
